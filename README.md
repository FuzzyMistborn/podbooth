...
4. Guests open the link, check their devices (mic/camera selections are saved for future sessions), enter their name, and join. In Chromium browsers, an optional **Enable local recording** button lets a participant pick a folder on disk up front — if granted, their tracks are written straight to files in that folder as they record instead of browser storage
5. Host clicks **REC** to start — every participant records entirely locally (no upload while capture is live); a live input level meter and clip indicator appear on each tile during recording. Recordings upload automatically once the recording stops
...
- **Local-first recording**: Recording is local-only while capture is live — nothing is sent to the server until the recording stops. Audio flushes every 5s; video MediaRecorder fires every 5s. Each chunk is written through to IndexedDB the instant it's produced (or, if the participant opted into a local folder, appended to a real file on disk via the File System Access API — see below). Once stopped, every chunk for a track is uploaded in order with retries, and `finalize` only fires once all of that track's chunks have landed — no race between the final chunk and assembly.
- **Optional local folder (File System Access)**: Chromium-only, opt-in per participant from the prejoin screen. If granted, each track is written straight to one file in the chosen folder as it's captured — named after the participant, track type, and recording epoch — rather than many chunk records in IndexedDB. The file is flushed to disk periodically during recording (not held open until the end) so a crash loses at most a few seconds of data instead of the whole take, then uploaded whole once recording stops. Firefox/Safari (no File System Access support) and participants who don't opt in fall back to the IndexedDB path automatically.
